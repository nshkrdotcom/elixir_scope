...
defmodule ElixirScope.Storage.DataAccess do
  ...
  def query_by_time_range(%__MODULE__{} = storage, start_time, end_time, opts \\ []) do
    ...
    try do
      ...
      event_ids =
        get_events_in_time_range(storage.temporal_index, start_time, end_time, limit, order)
  ...
  def query_by_process(%__MODULE__{} = storage, pid, opts \\ []) do
    ...
    try do
      ...
      event_ids =
        :ets.lookup(storage.process_index, pid)
        |> Enum.map(&elem(&1, 1))
        |> Enum.take(limit)
  ...
  def query_by_function(%__MODULE__{} = storage, module, function, opts \\ []) do
    ...
    try do
      ...
      event_ids =
        :ets.lookup(storage.function_index, {module, function})
        |> Enum.map(&elem(&1, 1))
        |> Enum.take(limit)
  ...
  def query_by_correlation(%__MODULE__{} = storage, correlation_id, opts \\ []) do
    ...
    try do
      ...
      event_ids =
        :ets.lookup(storage.correlation_index, correlation_id)
        |> Enum.map(&elem(&1, 1))
        |> Enum.take(limit)
  ...
  def get_events_since(since_timestamp) do
    ...
    case get_default_storage() do
      {:ok, storage} ->
        case query_by_time_range(storage, since_timestamp, Utils.monotonic_timestamp(),
               limit: 10_000
             ) do
          {:ok, events} -> events
          {:error, _} -> []
  ...
  defp extract_function_info(%Events.FunctionExecution{module: module, function: function})
       when not is_nil(module) and not is_nil(function),
       do: {module, function}
