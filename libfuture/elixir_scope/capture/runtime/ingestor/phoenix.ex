...
defmodule ElixirScope.Capture.Runtime.Ingestor.Phoenix do
  ...
  """
  @spec ingest_phoenix_request_start(RingBuffer.t(), term(), binary(), binary(), map(), binary()) ::
          ingest_result()
  ...
  """
  @spec ingest_phoenix_request_complete(RingBuffer.t(), term(), integer(), binary(), number()) ::
          ingest_result()
  def ingest_phoenix_request_complete(
        buffer,
        correlation_id,
        status_code,
        content_type,
  ...
  """
  @spec ingest_phoenix_controller_entry(RingBuffer.t(), term(), module(), atom(), map()) ::
          ingest_result()
  ...
  """
  @spec ingest_phoenix_controller_exit(RingBuffer.t(), term(), module(), atom(), term()) ::
          ingest_result()
  ...
  def ingest_phoenix_action_params(buffer, action_name, conn, params) do
    event =
      Events.new_event(:phoenix_action_params, %{
  ...
  def ingest_phoenix_action_start(buffer, action_name, conn) do
    event =
      Events.new_event(:phoenix_action_start, %{
        action_name: action_name,
        conn: Utils.truncate_data(conn)
      })
  ...
  def ingest_phoenix_action_success(buffer, action_name, conn, result) do
    event =
      Events.new_event(:phoenix_action_success, %{
        action_name: action_name,
        conn: Utils.truncate_data(conn),
        result: Utils.truncate_data(result)
      })
  ...
  """
  @spec ingest_phoenix_action_error(RingBuffer.t(), atom(), term(), atom(), term()) ::
          ingest_result()
  def ingest_phoenix_action_error(buffer, action_name, conn, kind, reason) do
    event =
      Events.new_event(:phoenix_action_error, %{
        action_name: action_name,
        conn: Utils.truncate_data(conn),
        kind: kind,
  ...
  def ingest_phoenix_action_complete(buffer, action_name, conn) do
    event =
      Events.new_event(:phoenix_action_complete, %{
        action_name: action_name,
        conn: Utils.truncate_data(conn)
      })
