...
defmodule ElixirScope.Capture.Runtime.Ingestor.LiveView do
  ...
  """
  @spec ingest_liveview_mount_start(RingBuffer.t(), term(), module(), map(), map()) ::
          ingest_result()
  def ingest_liveview_mount_start(buffer, correlation_id, module, params, session) do
    event =
      Events.new_event(
        :liveview_mount_start,
        %{
          module: module,
  ...
  """
  @spec ingest_liveview_handle_event_start(RingBuffer.t(), term(), binary(), map(), map()) ::
          ingest_result()
  ...
  """
  @spec ingest_liveview_handle_event_complete(
          RingBuffer.t(),
          term(),
          binary(),
          map(),
  ...
  def ingest_liveview_handle_event_complete(
        buffer,
        correlation_id,
        event,
        params,
  ...
  def ingest_liveview_assigns(buffer, callback_name, socket) do
    event =
      Events.new_event(:liveview_assigns_change, %{
  ...
  def ingest_liveview_event(buffer, event, params, socket) do
    event_struct =
      Events.new_event(:liveview_event, %{
        event: event,
        params: Utils.truncate_data(params),
        socket: Utils.truncate_data(socket)
      })
  ...
  def ingest_liveview_callback(buffer, callback_name, socket) do
    event =
      Events.new_event(:liveview_callback, %{
        callback_name: callback_name,
        socket: Utils.truncate_data(socket)
      })
  ...
  def ingest_liveview_callback_success(buffer, callback_name, socket, result) do
    event =
      Events.new_event(:liveview_callback_success, %{
        callback_name: callback_name,
        socket: Utils.truncate_data(socket),
        result: Utils.truncate_data(result)
      })
  ...
  """
  @spec ingest_liveview_callback_error(RingBuffer.t(), atom(), term(), atom(), term()) ::
          ingest_result()
  def ingest_liveview_callback_error(buffer, callback_name, socket, kind, reason) do
    event =
      Events.new_event(:liveview_callback_error, %{
        callback_name: callback_name,
        socket: Utils.truncate_data(socket),
        kind: kind,
