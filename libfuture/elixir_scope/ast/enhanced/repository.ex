...
defmodule ElixirScope.AST.Enhanced.Repository do
      ...
      :ok ->
        ...
        try do
          ...
          case :ets.lookup(:ast_modules_enhanced, module_data.module_name) do
            ...
            [] ->
              Logger.error(
                "Repository: Storage verification failed - module not found after insert!"
              )
          end
          ...
          Logger.debug(
            "Repository: Module storage completed successfully. Total modules: #{new_stats.modules_stored}"
          )
          ...
          e ->
            ...
            Logger.error(
              "Repository: Exception stacktrace: #{Exception.format_stacktrace(__STACKTRACE__)}"
            )
  ...
  def handle_call({:get_module, module_name}, _from, state) do
    ...
    case :ets.lookup(:ast_modules_enhanced, module_name) do
      ...
      [] ->
        ...
        all_modules =
          :ets.tab2list(:ast_modules_enhanced)
  ...
  def handle_call({:query_modules_by_file, file_path}, _from, state) do
    modules =
      :ets.lookup(:ast_module_by_file, file_path)
  ...
  def handle_call({:query_functions, criteria}, _from, state) do
    ...
    Logger.debug(
      "Function query completed in #{query_time_us}µs, returned #{length(functions)} results"
    )
  ...
  def handle_call({:find_references, module_name, function_name, arity}, _from, state) do
    ...
    references =
      :ets.lookup(:ast_calls_by_target, target_key)
  ...
  defp clear_all_tables do
    tables = [
      ...
      :ast_metadata,
      :ast_module_by_file,
      :ast_function_by_name,
      :ast_function_by_complexity,
      :ast_calls_by_target
    ]
  ...
  defp store_function_ast_nodes(function_data) do
    ...
    ast_node_id =
      "#{function_data.module_name}:#{function_data.function_name}:#{function_data.arity}:def"
  ...
  defp execute_function_query(criteria) do
    ...
    all_functions =
      :ets.tab2list(:ast_functions_enhanced)
  ...
  defp apply_function_sorting(functions, criteria) do
    case Map.get(criteria, :sort) do
      {:desc, :complexity} ->
        Enum.sort_by(
          functions,
          fn f -> get_in(f.complexity_metrics, [:combined_complexity]) || 1.0 end,
          :desc
        )
