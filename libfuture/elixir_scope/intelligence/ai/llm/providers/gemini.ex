...
defmodule ElixirScope.Intelligence.AI.LLM.Providers.Gemini do
  ...
  defp make_gemini_request(prompt, analysis_type) do
    ...
    if Mix.env() == :test and not test_mode_allows_http?() do
      ...
      Response.error("Gemini API not available in test mode without valid API key", :gemini, %{
      ...
      case get_api_key() do
        ...
        api_key when byte_size(api_key) < 10 ->
          ...
          Response.error("Gemini API key appears to be invalid", :gemini, %{
            analysis_type: analysis_type
          })
  ...
  defp perform_request_with_retry(url, body, headers, timeout, analysis_type, retries_left) do
    ...
    case HTTPoison.post(url, body, headers, timeout: timeout, recv_timeout: timeout) do
      ...
      {:ok, %HTTPoison.Response{status_code: 429, body: error_body} = response}
      when retries_left > 0 ->
        # Rate limiting - wait and retry
        Logger.warning(
          "Gemini: Rate limited (status: 429), retrying in 2 seconds (#{retries_left} retries left)"
        )
      ...
      when status_code >= 500 and retries_left > 0 ->
        ...
        Logger.warning(
          "Gemini: Server error #{status_code}, retrying in 1 second (#{retries_left} retries left)"
        )
      ...
      {:error, %HTTPoison.Error{reason: :timeout}} ->
        ...
        Response.error("Request timeout - check network connection and API key", :gemini, %{
          analysis_type: analysis_type
        })
      ...
      {:error, %HTTPoison.Error{reason: :nxdomain}} ->
        ...
        Response.error("DNS resolution failed - check network connection", :gemini, %{
          analysis_type: analysis_type
        })
  ...
  defp build_api_url do
    ...
    Logger.debug(
      "Gemini: API key present: #{if api_key, do: "yes (length: #{byte_size(api_key)})", else: "no"}"
    )
  ...
  defp parse_success_response(response_body, analysis_type) do
    ...
    case Jason.decode(response_body) do
      ...
      {:ok, response} ->
        ...
        Response.error("Unexpected response format: #{inspect(response)}", :gemini, %{
          analysis_type: analysis_type
        })
  ...
  defp parse_error_response(status_code, error_body, analysis_type) do
    ...
    error_message =
      case Jason.decode(error_body) do
        ...
        {:ok, decoded} ->
          Logger.debug(
            "Gemini: Decoded response without standard error format: #{inspect(decoded, limit: :infinity)}"
          )
