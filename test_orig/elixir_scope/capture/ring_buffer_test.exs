defmodule ElixirScope.Capture.RingBufferTest do
  ...
  describe "write/2 and read/2" do
    ...
    test "writes multiple events and reads them in order", %{buffer: buffer} do
      ...
      # Read all events
      {read_events, _} =
        Enum.reduce(0..4, {[], 0}, fn _, {acc, pos} ->
  ...
  describe "read_batch/3" do
    ...
    test "reads all available events when count exceeds available", %{
      buffer: buffer,
      events: events
    } do
      ...
      reader_tasks =
        for _i <- 1..2 do
          Task.async(fn ->
            ...
            total_read =
              Enum.reduce_while(1..max_attempts, 0, fn _attempt, acc_read ->
  ...
  describe "memory usage" do
    test "memory usage stays bounded" do
      # Smaller buffer
      {:ok, buffer} = RingBuffer.new(size: 512)
