defmodule ElixirScope.Foundation.Integration.ConfigEventsTelemetryTest do
  @moduledoc """
  ...
  alias ElixirScope.Foundation.{Config, Telemetry}
  ...
  describe "config → events → telemetry integration" do
    test "config update creates event and updates telemetry" do
      ...
      config_events = Enum.filter(events, fn event ->
    ...
    test "multiple config updates create separate events with correlation" do
      ...
      _correlation_id = ElixirScope.Foundation.Utils.generate_correlation_id()
      ...
      recent_cutoff = System.monotonic_time() - 10_000 # Last 10 seconds
      recent_events = Enum.filter(all_events, fn event ->
        event.timestamp > recent_cutoff
      end)
      
      # Should have at least one event for each update
      dev_events = Enum.filter(recent_events, fn event ->
  ...
  describe "error propagation integration" do
    test "config validation errors create error events" do
      ...
      result = Config.update([:ai, :planning, :sampling_rate], 2.0) # Invalid: > 1.0
  ...
  describe "performance integration" do
    test "high-frequency config updates maintain performance" do
      ...
      updates = for i <- 1..10 do
        rate = 0.1 + (i * 0.05) # 0.15, 0.20, 0.25, etc.
      ...
      Process.sleep(200) # Allow time for event/telemetry processing
      
      # Verify events were created
      query = %{event_type: :config_updated}
      {:ok, events} = EventStore.query(query)
      
      recent_cutoff = System.monotonic_time() - 10_000_000 # Last 10 seconds in microseconds
