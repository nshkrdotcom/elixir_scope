defmodule ElixirScope.Foundation.Integration.EndToEndDataFlowTest do
  @moduledoc """
  ...
  describe "complete event lifecycle" do
    test "event creation → storage → query → correlation → telemetry" do
      ...
      {stored_event_ids, _} = 
        Enum.map_reduce(events_data, nil, fn event_data, parent_event_id ->
          ...
          {event_id, event_id}  # Return event_id and set it as next parent
        end)
      ...
      end_time = first_event.timestamp + 10_000_000 # 10 seconds later
      ...
      session_in_range = Enum.filter(time_filtered_events, fn event ->
      ...
      login_events = Enum.filter(action_events, fn event ->
        event.data.action == "user_login"
      end)
    ...
    test "configuration-driven event processing" do
      ...
      _post_config_events = for i <- 4..6 do
      ...
      baseline_in_results = Enum.filter(all_test_events, fn event ->
        event.data.phase == "baseline"
      end)
      
      post_config_in_results = Enum.filter(all_test_events, fn event ->
  ...
  describe "error handling across components" do
    test "errors propagate correctly through the system" do
      ...
      invalid_result = Config.update([:ai, :planning, :sampling_rate], 5.0) # Invalid > 1.0
  ...
  describe "data consistency" do
    test "events maintain consistency across operations" do
      ...
      event_sequence = 
        Enum.reduce(1..5, [], fn i, acc ->
          parent_id = case acc do
            [] -> nil  # First event has no parent
            [{_prev_event_id, prev_event} | _] -> prev_event.event_id  # Use previous event's ID
          end
      ...
      individual_events = for event_id <- stored_event_ids do
      ...
      our_type_events = Enum.filter(type_events, fn event ->
        event.correlation_id == correlation_id
      end)
