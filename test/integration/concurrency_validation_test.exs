defmodule ElixirScope.Foundation.ConcurrencyValidationTest do
  ...
  describe "Registry Infrastructure" do
    test "ProcessRegistry provides namespace isolation", %{test_ref: test_ref} do
      ...
      case ProcessRegistry.lookup(:production, :config_server) do
        {:ok, prod_config_pid} ->
          assert test_config_pid != prod_config_pid, "Test and production should have different PIDs"
        :error ->
    ...
    test "ServiceRegistry provides safe service interaction", %{namespace: namespace} do
      ...
      assert info.total_services >= 2  # At least config_server and event_store
  ...
  describe "Service Isolation" do
    test "ConfigServer operates independently per namespace", %{namespace: namespace} do
      ...
      case ServiceRegistry.lookup(:production, :config_server) do
        {:ok, prod_pid} ->
          ...
          assert prod_debug in [false, true]  # Accept either since production might be modified by other tests
    ...
    test "EventStore operates independently per namespace", %{namespace: namespace} do
      # Create a test event
      event = ElixirScope.Foundation.Events.debug_new_event(:test_event, %{data: "test"})
  ...
  describe "Concurrent Operations" do
    test "handles multiple concurrent config reads", %{namespace: namespace} do
      operations = [
        fn _ns -> with_service(namespace, :config_server, fn pid -> 
      ...
      assert results.duration_ms < 1000  # Should complete within 1 second
    ...
    test "serializes concurrent config updates correctly", %{namespace: namespace} do
      ...
      with_service(namespace, :config_server, fn pid ->
        ...
        assert is_boolean(final_value), "Expected boolean but got #{final_value} - updates may not be properly serialized"
      end)
    end

    test "handles concurrent event storage", %{namespace: namespace} do
      events = for i <- 1..50 do
        ElixirScope.Foundation.Events.debug_new_event(:concurrent_test, %{index: i})
      end
  ...
  describe "Performance Validation" do
    test "system remains responsive under load", %{namespace: namespace} do
      ...
      operations = [
        ...
        fn _ns -> with_service(namespace, :event_store, fn pid ->
          event = ElixirScope.Foundation.Events.debug_new_event(:load_test, %{})
          GenServer.call(pid, {:store_event, event})
        end) end
      ]
      ...
      assert results.ops_per_second > 100  # At least 100 ops/second
      assert results.duration_ms < 5000     # Complete within 5 seconds
    end
  end
end 