defmodule ElixirScope.Foundation.Property.ShrinkingBottleneckTest do
  ...
  property "SHRINKING TEST: Simple failure to measure framework overhead" do
    ...
    result =
      try do
        ...
        e ->
          timestamp_log("Caught expected failure: #{inspect(e)}")
          :shrinking_measured
      end

    # The test passes - we just measured the overhead
    assert result in [:test_completed, :shrinking_measured]
  end
  ...
  property "SHRINKING TEST: Complex generators to measure overhead" do
      ...
      try do
        ...
        operations_generator =
          StreamData.list_of(
            ...
            min_length: 3,
            # This should trigger complex shrinking
            max_length: 8
          )

        check all(
                operations <- operations_generator,
                # Very limited to avoid excessive shrinking
                max_runs: 3
              ) do
          ...
          if length(operations) > 5 do
            timestamp_log(
              "INTENTIONAL FAILURE: #{length(operations)} operations > 5 - triggering complex shrinking"
            )
        ...
        e ->
          timestamp_log("Caught expected complex shrinking failure: #{inspect(e)}")
          :complex_shrinking_measured
      end
    ...
    # The test passes - we just measured the overhead
    assert result in [:test_completed, :complex_shrinking_measured]
  end
  ...
  property "SHRINKING TEST: Service restart overhead during failures" do
    ...
    result =
      try do
        check all(
                restart_trigger <- StreamData.boolean(),
                max_runs: 5
              ) do
          test_start = timestamp_ms()
        ...
        e ->
          timestamp_log("Caught expected restart overhead failure: #{inspect(e)}")
          :restart_overhead_measured
      end

    # The test passes - we just measured the overhead
    assert result in [:test_completed, :restart_overhead_measured]
  end
  ...
  property "SHRINKING TEST: Replicate original telemetry test structure" do
    ...
    result =
      try do
        ...
        mixed_operation_sequence_generator =
          StreamData.list_of(
            StreamData.one_of([
              StreamData.tuple({
                StreamData.constant(:gauge),
        ...
        check all(
                operations <- mixed_operation_sequence_generator,
                max_runs: 2
              ) do
          run_start = timestamp_ms()
        ...
        e ->
          timestamp_log("Caught expected replication failure: #{inspect(e)}")
    ...
    # The test passes - we just measured the overhead
    assert result in [:test_completed, :replication_measured]
  end
