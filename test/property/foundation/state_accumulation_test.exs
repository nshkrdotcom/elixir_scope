defmodule ElixirScope.Foundation.Property.StateAccumulationTest do
  ...
  test "STATE ACCUMULATION: Reproduce 32-second delay through metric buildup" do
    ...
    # Simulate growing metric counts
    ...
    for iteration_count <- iterations do
      ...
      timestamp_log(
        "=== ITERATION #{iteration_count}: Adding #{iteration_count} metrics ===",
        should_log
      )
      ...
      if total_iter_duration > 5000 do
        timestamp_log(
          "*** FOUND THE BOTTLENECK: #{iteration_count} operations took #{total_iter_duration}ms ***",
          true
        )
      ...
      if metrics_duration > 1000 do
        timestamp_log(
          "*** GET_METRICS IS SLOW: #{metrics_duration}ms with #{total_metrics} metrics ***",
          true
        )
      end
      ...
      if verify_duration > 1000 do
        timestamp_log(
          "*** VERIFICATION IS SLOW: #{verify_duration}ms with #{total_metrics} metrics ***",
          true
        )
      end
    end
  ...
  test "STATE RESET: Compare performance with and without proper cleanup" do
    ...
    if no_reset_duration > reset_duration * 2 do
      timestamp_log(
        "*** STATE ACCUMULATION CONFIRMED: #{no_reset_duration}ms vs #{reset_duration}ms ***",
        true
      )
    end
  ...
  defp verify_metrics_structure(metrics) when is_map(metrics) do
    Enum.each(metrics, fn {key, value} ->
      ...
      case value do
        ...
        # Recursive call
        v when is_map(v) -> verify_metrics_structure(v)
