defmodule ElixirScope.Foundation.Property.ErrorPropertiesTest do
  ...
  defp stacktrace_entry_generator do
    one_of([
      ...
      tuple({
        atom(:alphanumeric),  # module
        atom(:alphanumeric),  # function
        integer(0..10),       # arity
        list_of(tuple({atom(:alphanumeric), term()}))  # location
      }),
  ...
  property "Error.new/3 with random messages never crashes regardless of input" do
    check all error_type <- error_type_generator(),
  ...
  property "Error.wrap_error/4 with nested error chains maintains error hierarchy" do
    check all original_type <- error_type_generator(),
              wrapper_type <- error_type_generator(),
              original_context <- context_generator(),
              wrapper_context <- context_generator() do
      ...
      result = Error.wrap_error({:error, original_error}, wrapper_type, "Wrapped error", wrapper_opts)
  ...
  property "Error.retry_delay/2 exponential backoff never exceeds maximum delay cap" do
    check all attempt <- integer(0..20) do
      error = Error.new(:network_error)  # Known to use exponential backoff
      delay = Error.retry_delay(error, attempt)
      
      assert is_integer(delay)
      assert delay >= 0
      assert delay <= 30_000  # Maximum cap from implementation
    end
  end
  ...
  property "Error serialization and deserialization roundtrip preserves all fields" do
    check all error_type <- error_type_generator(),
              ...
              correlation_id <- one_of([constant(nil), string(:alphanumeric)]) do
      opts = [
        context: context || %{},
