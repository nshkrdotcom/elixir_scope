defmodule ElixirScope.Foundation.Property.TelemetryAggregationPropertiesTest do
  # Telemetry operations affect shared state
        ...
        _pid ->
          ...
          case GenServer.call(TelemetryService, :clear_metrics, 5000) do
            ...
            _error ->
              ...
              case TelemetryService.start_link([]) do
                {:ok, _} ->
                  GenServer.call(TelemetryService, :clear_metrics, 5000)
  ...
  property "Telemetry.emit_gauge/2 with any numeric value maintains type consistency" do
    check all(
            path <- metric_path_generator(),
            value <- numeric_value_generator(),
            metadata <- metric_metadata_generator()
          ) do
  ...
  property "Telemetry metric aggregation is commutative and associative" do
    check all(
            increments <-
              StreamData.list_of(counter_increment_generator(), min_length: 2, max_length: 5),
            max_runs: 3
          ) do
  ...
  property "TelemetryService restart preserves accumulated metric state" do
    check all(
            operations <-
              list_of(
                tuple(
                  {:counter, metric_path_generator(), counter_increment_generator(),
                   metric_metadata_generator()}
                ),
                min_length: 3,
                max_length: 10
              )
  ...
  property "Telemetry collection with high-frequency updates maintains accuracy" do
    check all(
      ...
      expected_total =
        Enum.reduce(1..update_count, 0, fn i, acc ->
  ...
  property "Metric transformation preserves numerical relationships" do
    check all(
      ...
      retrieved_base =
        case get_in(base_result, path) do
      ...
      retrieved_mult =
        case get_in(mult_result, path) do
          %{measurements: %{gauge: value}} ->
            value
  ...
  property "TelemetryService concurrent metric updates maintain consistency" do
    check all(
            task_count <- integer(5..20),
            operations_per_task <- integer(5..15)
          ) do
      # Use unique path to avoid interference
          ...
          Task.async(fn ->
            task_total =
              for op_id <- 1..operations_per_task do
                ...
                # Unique increments
                increment = task_id + op_id

                :ok =
                  TelemetryService.emit_counter(path, increment, %{
                    task_id: task_id,
                    op_id: op_id,
                    increment: increment
                  })
