defmodule ElixirScope.Foundation.Property.PerformanceDebugTest do
  ...
  test "Performance profiling: Service startup time" do
    ...
    if Process.whereis(TelemetryService) do
      {_, duration} =
        time_operation("Service stop", fn ->
    ...
    {result, startup_duration} =
      time_operation("Service startup", fn ->
  ...
  test "Performance profiling: Property test shrinking simulation" do
    ...
    for attempt <- 1..5 do
      ...
      {metrics_result, get_duration} =
        time_operation("Get metrics for verification", fn ->
          TelemetryService.get_metrics()
        end)
      ...
      {_, verify_duration} =
        time_operation("Verify metrics structure", fn ->
      ...
      timestamp_log(
        "Attempt #{attempt}: exec=#{exec_duration}ms, get=#{get_duration}ms, verify=#{verify_duration}ms, total=#{attempt_total}ms"
      )
  ...
  test "Performance profiling: Service restart during failure" do
    ...
    {_, wait_duration} =
      time_operation("Wait period", fn ->
        Process.sleep(100)
      end)

    # Restart service
    {_, start_duration} =
      time_operation("Service restart", fn ->
        {:ok, _pid} = TelemetryService.start_link([])
      end)

    # Initial operations post-restart - FIXED: handle return values
    {_, init_duration} =
      time_operation("Post-restart operations", fn ->
    ...
    timestamp_log(
      "Restart breakdown: stop=#{stop_duration}ms, wait=#{wait_duration}ms, start=#{start_duration}ms, init=#{init_duration}ms"
    )
  ...
  test "Performance profiling: Complex metrics structure verification" do
    ...
    {_, setup_duration} =
      time_operation("Setup complex metrics", fn ->
        for i <- 1..10 do
    ...
    {metrics_result, get_duration} =
      time_operation("Get complex metrics", fn ->
        TelemetryService.get_metrics()
      end)
    ...
    {_, verify_duration} =
      time_operation("Verify complex structure", fn ->
        verify_complex_metrics_structure(metrics)
      end)

    timestamp_log(
      "Setup: #{setup_duration}ms, Get: #{get_duration}ms, Verify: #{verify_duration}ms"
    )
  ...
  property "Minimal property test to isolate shrinking overhead" do
    ...
    check all(
      ...
      # Minimal work
      {_, work_duration} =
        time_operation("Minimal work", fn ->
  ...
  defp verify_complex_metrics_structure(metrics) do
    ...
    if total_duration > 100 do
      timestamp_log(
        "SLOW STRUCTURE VERIFICATION: #{total_duration}ms for #{map_size(metrics)} entries"
      )
    end
