    ...
    test "for existing Error.t correctly preserves and enhances context" do
      ...
      result = Error.wrap_error(
        ...
        context: %{
          ...
          nested: %{
            rules: ["max_length"],  # This should be added
      ...
      assert wrapped_error.error_type == :invalid_config_value  # Original error type preserved
      assert wrapped_error.message == "Original validation failed"  # Original message preserved
  ...
  describe "Error.format_stacktrace/1" do
    ...
    test "correctly formats entries not matching {M,F,A,L} tuple" do
      ...
      irregular_stacktrace = [
        {Module, :function, 2, [file: "test.ex", line: 42]},  # Standard format
        "string_entry",          # String instead of tuple
        {:atom_only},           # Single atom tuple
        42,                     # Non-tuple entry
      ]
