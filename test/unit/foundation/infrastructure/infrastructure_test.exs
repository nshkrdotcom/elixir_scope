defmodule ElixirScope.Foundation.InfrastructureTest do
  ...
  describe "execute_protected/4 with circuit breaker" do
    setup do
      ...
      :ok = ElixirScope.Foundation.Infrastructure.CircuitBreaker.start_fuse_instance(:test_cb)
      :ok
    end
  ...
  describe "execute_protected/4 with rate limiter" do
    ...
    test "returns error when rate limit exceeded" do
      ...
      assert {:error, %ElixirScope.Foundation.Types.Error{error_type: :rate_limit_exceeded}} =
               Infrastructure.execute_protected(:rate_limiter, config, operation)
    end
