...
defmodule ElixirScope.ASTRepository.RepositoryTest do
  ...
  describe "Module storage and retrieval" do
    setup do
      ...
      module_data =
        ModuleData.new(:TestModule, sample_ast,
    ...
    test "update returns error for non-existent module", %{repository: repository} do
      ...
      assert {:error, :not_found} =
               Repository.update_module(repository, :NonExistentModule, update_fn)
    end
  end
  ...
  describe "Function storage and retrieval" do
    setup do
      ...
      function_data =
        FunctionData.new(function_key, function_ast,
    ...
    test "can store and retrieve function data", %{
  ...
  describe "Runtime correlation" do
    setup do
      ...
      module_data =
        ModuleData.new(:TestModule, sample_ast, correlation_metadata: correlation_metadata)
    ...
    test "can correlate runtime events", %{
      repository: repository,
    ...
    test "returns error for unknown correlation ID", %{repository: repository} do
      ...
      assert {:error, :correlation_not_found} =
               Repository.correlate_event(repository, runtime_event)
    end
  end
  ...
  describe "Instrumentation points" do
    setup do
      ...
      module_data =
        ModuleData.new(:InstrumentedModule, sample_ast,
    ...
    test "returns error for non-existent instrumentation point", %{repository: repository} do
      assert {:error, :not_found} =
               Repository.get_instrumentation_points(repository, "non_existent_node")
    end
  end
end
