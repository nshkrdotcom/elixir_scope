defmodule ElixirScope.Foundation.TestProcessManager do
  @moduledoc """
  ...
  def reset_all_service_states do
    if Application.get_env(:elixir_scope, :test_mode, false) do
      errors = Enum.reduce(@foundation_services, [], fn service_module, acc_errors ->
  ...
  def all_services_available? do
    results = Enum.map(@foundation_services, fn service_module ->
  ...
  def get_services_status do
    Enum.reduce(@foundation_services, %{}, fn service_module, acc ->
      ...
      status = case service_module.status() do
  ...
  defp get_service_process_name(service_module) do
    if Application.get_env(:elixir_scope, :test_mode, false) do
      ...
      found_process = Enum.find(registered_processes, fn name ->
  ...
  defp cleanup_orphaned_processes do
    ...
    essential_processes = [
      ...
      :"ElixirScope.Foundation.TaskSupervisor"
    ]
    
    foundation_related = Enum.filter(registered_processes, fn name ->
      name_string = Atom.to_string(name)
      String.contains?(name_string, "ElixirScope.Foundation") and
      name not in essential_processes and
      not String.contains?(name_string, "Supervisor")  # Don't kill any supervisor processes
    end)
