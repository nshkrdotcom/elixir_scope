...
defmodule ElixirScope.Intelligence.AI.CodeAnalyzer do
  ...
  def analyze_project(project_path) do
    ...
    %{
      ...
      phoenix_modules:
        count_by_type(module_analyses, :phoenix_controller) +
  ...
  def analyze_function(function_code) do
    ...
    %{
      ...
      recommended_instrumentation:
        recommend_function_instrumentation(complexity, performance_indicators)
    }
  end
  ...
  defp extract_pubsub_patterns(code_samples) do
    Enum.flat_map(code_samples, fn code ->
      ...
      Macro.prewalk(ast, [], fn
        ...
        {{:., _, [{:__aliases__, _, [:Phoenix, :PubSub]}, :broadcast]}, _, [pubsub, topic, message]} =
            node,
        acc ->
          pattern = %{
  ...
  defp calculate_estimated_overhead(module_analyses) do
    ...
    instrumented_modules =
      Enum.count(module_analyses, &(&1.recommended_instrumentation != :minimal))

    # Estimate based on complexity and number of instrumented modules
    # 0.1% per module
    base_overhead = instrumented_modules * 0.001
    # Based on complexity
    complexity_overhead = total_complexity * 0.0001

    # Cap at 5%
    min(base_overhead + complexity_overhead, 0.05)
  end
  ...
  defp extract_topic_pattern({:<<>>, _, parts}) do
    ...
    case parts do
      ...
      [prefix, {:"::", _, [{{:., _, [Kernel, :to_string]}, _, [_expr]}, {:binary, _, _}]}]
  ...
  defp extract_message_type({type, _}) when is_atom(type), do: type
