...
defmodule ElixirScope.Capture.Runtime.TemporalStorage do
  ...
  defstruct [
    :storage_id,
    # ETS table for events: {timestamp, event_id, event}
    :events_table,
    # ETS table for AST correlation: {ast_node_id, [event_ids]}
    :ast_index,
    # ETS table for correlation: {correlation_id, [event_ids]}
    :correlation_index,
    # Storage statistics
    :stats,
  ...
  defp get_events_in_range_impl(state, start_time, end_time) do
    try do
      ...
      events =
        :ets.select(state.events_table, [
          {{:"$1", :"$2", :"$3"}, [{:andalso, {:>=, :"$1", start_time}, {:"=<", :"$1", end_time}}],
           [:"$3"]}
        ])
  ...
  defp get_events_for_correlation_impl(state, correlation_id) do
    try do
      ...
      event_ids =
        :ets.lookup(state.correlation_index, correlation_id)
        |> Enum.map(fn {_, event_id} -> event_id end)
  ...
  defp calculate_stats(state) do
    memory_usage =
      :ets.info(state.events_table, :memory) +
