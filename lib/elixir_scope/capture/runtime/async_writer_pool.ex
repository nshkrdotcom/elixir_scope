...
defmodule ElixirScope.Capture.Runtime.AsyncWriterPool do
  ...
  def handle_call({:scale_pool, new_size}, _from, state) do
    ...
    cond do
      new_size > current_size ->
        ...
        {new_workers, new_monitors} =
          start_additional_workers(additional_workers_needed, state.config)
  ...
  def handle_call(:get_metrics, _from, state) do
    ...
    worker_metrics =
      Enum.map(state.workers, fn worker_pid ->
        if Process.alive?(worker_pid) do
          try do
  ...
  defp handle_worker_death(worker_pid, reason, state) do
    ...
    if worker_pid in state.workers do
      Logger.warning(
        "AsyncWriter worker #{inspect(worker_pid)} died (#{inspect(reason)}), restarting..."
      )
  ...
  defp start_additional_workers(count, config) do
    workers_and_monitors =
      for _i <- 1..count do
        worker_config =
          Map.take(config, [:ring_buffer, :batch_size, :poll_interval_ms, :max_backlog])
