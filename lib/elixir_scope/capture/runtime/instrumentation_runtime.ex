...
defmodule ElixirScope.Capture.Runtime.InstrumentationRuntime do
  ...
  **Purpose**: Handles Phoenix framework events including HTTP requests, LiveView, and channels.
  ...
  defdelegate report_function_entry(function_name, arity, capture_args, correlation_id),
    to: CoreReporting

  defdelegate report_function_exit(correlation_id, return_value, duration_ns), to: CoreReporting

  defdelegate report_function_exit(function_name, arity, exit_type, return_value, correlation_id),
    to: CoreReporting
  ...
  defdelegate report_local_variable_snapshot(correlation_id, variables, line, source \\ :ast),
    to: ASTReporting

  defdelegate report_ast_variable_snapshot(correlation_id, variables, line, ast_node_id),
    to: ASTReporting

  defdelegate report_expression_value(correlation_id, expression, value, line, source \\ :ast),
    to: ASTReporting
  ...
  defdelegate report_ast_function_entry_with_node_id(
                module,
                function,
                args,
                correlation_id,
  ...
  defdelegate report_ast_function_exit_with_node_id(
                correlation_id,
                return_value,
                duration_ns,
                ast_node_id
  ...
  defdelegate report_ast_expression_value(correlation_id, expression, value, line, ast_node_id),
    to: ASTReporting

  defdelegate report_ast_line_execution(correlation_id, line, context, ast_node_id),
    to: ASTReporting

  defdelegate report_ast_pattern_match(
                correlation_id,
                pattern,
                value,
                match_success,
  ...
  defdelegate report_ast_branch_execution(
                correlation_id,
                branch_type,
                condition,
                branch_taken,
  ...
  defdelegate report_ast_loop_iteration(
                correlation_id,
                loop_type,
                iteration_count,
                current_value,
  ...
  defdelegate report_ast_correlation_performance(correlation_id, operation, duration_ns),
    to: ASTReporting

  # Phoenix integration functions
  defdelegate report_phoenix_request_start(correlation_id, method, path, params, remote_ip),
    to: PhoenixReporting

  defdelegate report_phoenix_request_complete(
                correlation_id,
                status_code,
                content_type,
                duration_ms
  ...
  defdelegate report_phoenix_controller_entry(correlation_id, controller, action, metadata),
    to: PhoenixReporting

  defdelegate report_phoenix_controller_exit(correlation_id, controller, action, result),
    to: PhoenixReporting

  defdelegate report_liveview_mount_start(correlation_id, module, params, session),
    to: PhoenixReporting

  defdelegate report_liveview_mount_complete(correlation_id, module, socket_assigns),
    to: PhoenixReporting

  defdelegate report_liveview_handle_event_start(correlation_id, event, params, socket_assigns),
    to: PhoenixReporting

  defdelegate report_liveview_handle_event_complete(
                correlation_id,
                event,
                params,
                before_assigns,
  ...
  defdelegate report_phoenix_channel_join_start(correlation_id, topic, payload, socket),
    to: PhoenixReporting

  defdelegate report_phoenix_channel_join_complete(correlation_id, topic, payload, result),
    to: PhoenixReporting

  defdelegate report_phoenix_channel_message_start(correlation_id, event, payload, socket),
    to: PhoenixReporting

  defdelegate report_phoenix_channel_message_complete(correlation_id, event, payload, result),
    to: PhoenixReporting

  defdelegate report_phoenix_action_params(action_name, conn, params, should_capture),
    to: PhoenixReporting

  defdelegate report_phoenix_action_start(action_name, conn, should_capture_state),
    to: PhoenixReporting
  ...
  defdelegate report_phoenix_action_complete(action_name, conn, should_capture_response),
    to: PhoenixReporting
  ...
  defdelegate report_liveview_callback_error(callback_name, socket, kind, reason),
    to: PhoenixReporting

  # Ecto integration functions
  defdelegate report_ecto_query_start(correlation_id, query, params, metadata, repo),
    to: EctoReporting

  defdelegate report_ecto_query_complete(correlation_id, query, params, result, duration_us),
    to: EctoReporting

  # GenServer integration functions
  defdelegate report_genserver_callback_start(callback_name, pid, capture_state),
    to: GenServerReporting

  defdelegate report_genserver_callback_success(callback_name, pid, result), to: GenServerReporting

  defdelegate report_genserver_callback_error(callback_name, pid, kind, reason),
    to: GenServerReporting

  defdelegate report_genserver_callback_complete(callback_name, pid, capture_state),
    to: GenServerReporting
