...
defmodule ElixirScope.Query.Legacy do
  ...
  def get_optimization_suggestions(_store, query) do
    ...
    suggestions =
      if strategy.index_type == :full_scan do
        [
          "Consider adding a pid, event_type, or time range filter to improve performance"
          | suggestions
        ]
      else
        suggestions
    ...
    suggestions =
      if not Keyword.has_key?(query, :limit) do
    ...
    suggestions =
      if strategy.index_type == :event_type and not Keyword.has_key?(query, :pid) do
    ...
    suggestions =
      if has_broad_time_range?(query) do
  ...
  defp build_metrics(strategy, results, execution_time, query) do
    ...
    events_scanned =
      case strategy.index_type do
        # Process index is very selective
        :process -> min(events_returned * 2, 50)
        # Event type is moderately selective
        :event_type -> events_returned * 3
        # Temporal can be selective
        :temporal -> events_returned * 2
        # Full scan is expensive
        :full_scan -> max(events_returned * 10, 100)
      end

    filter_efficiency =
      if events_scanned > 0 do
