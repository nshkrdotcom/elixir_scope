...
defmodule ElixirScope.AST.Enhanced.Synchronizer do
  ...
  alias ElixirScope.AST.Enhanced.{Repository, ProjectPopulator}
  ...
  def handle_call({:sync_file, file_path}, _from, state) do
    ...
    if File.exists?(file_path) do
      ...
      case ProjectPopulator.parse_and_analyze_file(file_path) do
        {:ok, module_data} ->
          Logger.debug(
            "Synchronizer: Parse successful for module: #{inspect(module_data.module_name)}"
          )
          ...
          try do
            Logger.debug(
              "Synchronizer: Attempting to store module in repository: #{inspect(state.repository)}"
            )
              ...
              :ok ->
                ...
                case Repository.get_module(state.repository, module_data.module_name) do
                  {:ok, retrieved_data} ->
                    ...
                    Logger.debug(
                      "Synchronizer: Retrieved module name: #{inspect(retrieved_data.module_name)}"
                    )

                  {:error, reason} ->
                    Logger.error(
                      "Synchronizer: Module retrieval verification failed: #{inspect(reason)}"
                    )
                end
            ...
            e ->
              Logger.error(
                "Synchronizer: Exception during repository storage: #{Exception.message(e)}"
              )

              Logger.error(
                "Synchronizer: Exception stacktrace: #{Exception.format_stacktrace(__STACKTRACE__)}"
              )
        ...
        {:error, {:ast_parsing_failed, file_path, reason}} ->
          Logger.warning(
            "Synchronizer: AST parsing failed for file: #{file_path}, reason: #{inspect(reason)}"
          )
  ...
  def handle_call({:sync_file_deletion, file_path}, _from, state) do
    ...
    case Repository.get_all_modules(state.repository) do
      {:ok, modules} ->
        ...
        modules_to_delete =
          Enum.filter(modules, fn module_data ->
          ...
          modules ->
            ...
            delete_results =
              Enum.map(modules, fn module_data ->
                Logger.debug(
                  "Synchronizer: Deleting module #{inspect(module_data.module_name)} for file: #{file_path}"
                )
            ...
            if Enum.all?(delete_results, &(&1 == :ok)) do
              Logger.debug(
                "Synchronizer: Successfully deleted #{length(modules)} module(s) for file: #{file_path}"
              )
  ...
  def handle_call({:sync_files, file_paths}, _from, state) do
    ...
    results =
      Enum.with_index(file_paths)
      |> Enum.map(fn {file_path, index} ->
        Logger.debug(
          "Synchronizer: Processing batch file #{index + 1}/#{length(file_paths)}: #{file_path}"
        )
        ...
        if File.exists?(file_path) do
          case ProjectPopulator.parse_and_analyze_file(file_path) do
            {:ok, module_data} ->
              Logger.debug(
                "Synchronizer: Batch parse successful for module: #{inspect(module_data.module_name)}"
              )
              ...
              try do
                case Repository.store_module(state.repository, module_data) do
                  :ok ->
                    Logger.debug(
                      "Synchronizer: Batch module stored successfully: #{inspect(module_data.module_name)}"
                    )

                    # Return success with data for batch tests
                    {:ok, module_data}

                  error ->
                    Logger.error(
                      "Synchronizer: Batch storage failed for #{file_path}: #{inspect(error)}"
                    )
                ...
                e ->
                  Logger.error(
                    "Synchronizer: Batch exception for #{file_path}: #{Exception.message(e)}"
                  )
            ...
            {:error, {:ast_parsing_failed, file_path, reason}} ->
              Logger.warning(
                "Synchronizer: Batch AST parsing failed for: #{file_path}, reason: #{inspect(reason)}"
              )
            ...
            {:error, {:parse_and_analyze_failed, reason}} ->
              Logger.warning(
                "Synchronizer: Batch parse and analyze failed for: #{file_path}, reason: #{inspect(reason)}"
              )
            ...
            {:error, reason} ->
              Logger.error(
                "Synchronizer: Batch unexpected error for #{file_path}: #{inspect(reason)}"
              )
  ...
  defp update_state_with_success(state) do
    %{state | sync_count: state.sync_count + 1, last_sync_time: DateTime.utc_now()}
  end

  defp update_state_with_error(state) do
    %{state | error_count: state.error_count + 1, last_sync_time: DateTime.utc_now()}
  end
end
