...
defmodule ElixirScope.AST.Enhanced.ProjectPopulator.ModuleAnalyzer do
  ...
  alias ElixirScope.AST.Enhanced.ProjectPopulator.{
    ASTExtractor,
    ComplexityAnalyzer,
    QualityAnalyzer,
    SecurityAnalyzer,
    ...
    try do
      analyzed_modules =
        if parallel do
          ...
          |> Enum.reduce(%{}, fn
            {:ok, {:ok, {module_name, module_data}}}, acc ->
              Map.put(acc, module_name, module_data)
  ...
  def analyze_single_module(parsed_file, generate_cfg, generate_dfg, generate_cpg, _timeout) do
    ...
    try do
      ...
      if module_name do
        ...
        analyzed_functions =
          Enum.reduce(functions, %{}, fn {func_name, arity, func_ast}, acc ->
            case analyze_single_function(
                   module_name,
                   func_name,
                   arity,
                   func_ast,
              ...
              {:error, reason} ->
                Logger.warning(
                  "Failed to analyze function #{module_name}.#{func_name}/#{arity}: #{inspect(reason)}"
                )
        ...
        enhanced_module = %EnhancedModuleData{
          ...
          complexity_metrics:
            ComplexityAnalyzer.calculate_module_complexity_metrics(
              parsed_file.ast,
              analyzed_functions
            ),
          quality_metrics:
            QualityAnalyzer.calculate_module_quality_metrics(parsed_file.ast, analyzed_functions),
          security_analysis:
            SecurityAnalyzer.perform_module_security_analysis(parsed_file.ast, analyzed_functions),
          performance_hints:
            OptimizationHints.generate_module_performance_hints(parsed_file.ast, analyzed_functions),
  ...
  defp analyze_single_function(
    ...
    try do
      ...
      enhanced_function = %EnhancedFunctionData{
        ...
        cpg_data: cpg_data,
        complexity_metrics:
          ComplexityAnalyzer.calculate_function_complexity_metrics(func_ast, cfg_data, dfg_data),
        performance_analysis:
          ComplexityAnalyzer.analyze_function_performance_characteristics(
            func_ast,
            cfg_data,
            dfg_data
          ),
        security_analysis:
          SecurityAnalyzer.analyze_function_security_characteristics(func_ast, cpg_data),
        optimization_hints:
          OptimizationHints.generate_function_optimization_hints(func_ast, cfg_data, dfg_data),
        created_at: DateTime.utc_now(),
        updated_at: DateTime.utc_now()
      }
