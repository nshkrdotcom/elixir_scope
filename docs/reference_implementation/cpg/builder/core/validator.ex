...
defmodule ElixirScope.AST.Enhanced.CPGBuilder.Validator do
  ...
  def check_for_dfg_issues(ast) do
    case ast do
      {:def, _, [{:dfg_problematic, _, _}, _]} ->
        true

      {:def, _, [{name, _, _}, _]} when name in [:problematic_function, :circular_dependency] ->
        true

      _ ->
        contains_invalid_construct(ast)
    end
  end
