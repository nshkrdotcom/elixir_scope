...
defmodule ElixirScope.Intelligence.AI.ComplexityAnalyzer do
  ...
  defp calculate_complexity_score(ast) do
    base_score = 1
    # Increase weight
    nesting_bonus = calculate_nesting_depth(ast) * 2
    branching_bonus = calculate_cyclomatic_complexity(ast)
    pattern_bonus = calculate_pattern_match_complexity(ast)
    # Add pipe operation penalty
    pipe_bonus = count_pipe_operations(ast)
    # Add enum operation penalty
    enum_bonus = count_enum_operations(ast)
  ...
  defp max_depth(ast, current_depth) do
    Macro.prewalk(ast, current_depth, fn
      ...
      {:if, _, [_condition, keyword_list]}, max_so_far ->
        ...
        else_max =
          if else_block, do: max_depth(else_block, current_depth + 1), else: current_depth + 1
      ...
      {:|>, _, [left, right]}, max_so_far ->
        ...
        # Pipes add one level
        right_max = max_depth(right, current_depth + 1)
  ...
  defp count_state_operations(ast) do
    Macro.prewalk(ast, 0, fn
      ...
      {:handle_call, _, _}, count ->
        {true, count + 1}

      {:handle_cast, _, _}, count ->
        {true, count + 1}

      {:handle_info, _, _}, count ->
        {true, count + 1}

      {:init, _, _}, count ->
        {true, count + 1}

      {:start_link, _, _}, count ->
        {true, count + 1}

      # State access patterns
      {{:., _, [:Map, :get]}, _, [_state | _]}, count ->
        {true, count + 1}

      {:reply, _, [_, _state]}, count ->
        {true, count + 1}

      {:noreply, _, [_state]}, count ->
        {true, count + 1}

      {:stop, _, [_, _state]}, count ->
        {true, count + 1}
