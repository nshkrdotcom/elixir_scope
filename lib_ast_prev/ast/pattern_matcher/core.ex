...
defmodule ElixirScope.AST.PatternMatcher.Core do
  ...
  alias ElixirScope.AST.PatternMatcher.{Types, Validators, Analyzers, Stats}

  @spec match_ast_pattern_with_timing(pid() | atom(), map()) ::
  ...
  @spec match_behavioral_pattern_with_timing(pid() | atom(), map(), atom()) ::
          {:ok, Types.pattern_result()} | {:error, term()}
  ...
  @spec match_anti_pattern_with_timing(pid() | atom(), map(), atom()) ::
          {:ok, Types.pattern_result()} | {:error, term()}
  ...
  defp match_behavioral_pattern_internal(repo, pattern_spec, pattern_library) do
    with {:ok, normalized_spec} <- Validators.normalize_pattern_spec(pattern_spec),
         {:ok, pattern_def} <-
           get_behavioral_pattern_definition(normalized_spec.pattern_type, pattern_library),
         {:ok, modules} <- get_all_modules_for_analysis(repo),
         {:ok, matches} <-
           Analyzers.analyze_behavioral_patterns(modules, pattern_def, normalized_spec) do
      result = %{
        matches: matches,
  ...
  defp match_anti_pattern_internal(repo, pattern_spec, pattern_library) do
    with {:ok, normalized_spec} <- Validators.normalize_pattern_spec(pattern_spec),
         {:ok, pattern_def} <-
           get_anti_pattern_definition(normalized_spec.pattern_type, pattern_library),
         {:ok, functions} <- get_all_functions_for_analysis(repo),
