...
defmodule ElixirScope.AST.InstrumentationMapper do
  ...
  @type instrumentation_strategy ::
          :function_boundary | :expression_trace | :line_execution | :variable_capture | :none
  ...
  """
  @spec map_instrumentation_points(ast_node(), keyword()) ::
  ...
  def map_instrumentation_points(ast, opts \\ []) do
    ...
    include_expressions =
      Keyword.get(opts, :include_expressions, instrumentation_level in [:comprehensive, :debug])
    ...
    try do
      ...
      instrumentation_points =
        traverse_ast_for_instrumentation(ast, context, %{
  ...
  def select_instrumentation_strategy(ast_node, context) do
    case ast_node do
      ...
      {:defmacro, _meta, _args} ->
        :function_boundary

      # Variable assignments - only for debug level (must come before function calls)
      {:=, _meta, _args} when context.instrumentation_level == :debug ->
        :variable_capture

      # Control flow - instrument for debugging
      {:if, _meta, _args} when context.instrumentation_level in [:comprehensive, :debug] ->
        :expression_trace

      {:case, _meta, _args} when context.instrumentation_level in [:comprehensive, :debug] ->
        :expression_trace

      {:cond, _meta, _args} when context.instrumentation_level in [:comprehensive, :debug] ->
        :expression_trace

      # Pipe operations - instrument for comprehensive+
      {:|>, _meta, _args} when context.instrumentation_level in [:comprehensive, :debug] ->
        :expression_trace
  ...
  def configure_instrumentation(instrumentation_point, opts \\ []) do
    ...
    strategy_config =
      case instrumentation_point.strategy do
        :function_boundary ->
          %{
            capture_entry: true,
            capture_exit: true,
  ...
  def estimate_performance_impact(instrumentation_points) do
    total_impact =
      Enum.reduce(instrumentation_points, 0.0, fn point, acc ->
        impact =
          case point.strategy do
            # Low impact
            :function_boundary -> 0.1
            # Medium impact
  ...
  def optimize_for_performance(instrumentation_points, opts \\ []) do
    ...
    {function_points, other_points} =
      Enum.split_with(instrumentation_points, fn point ->
    ...
    {optimized_other, _} =
      Enum.reduce_while(sorted_other, {[], 0.0}, fn point, {acc, current_impact} ->
        point_impact =
          case point.strategy do
            :expression_trace -> 0.3
            :line_execution -> 0.2
  ...
  defp traverse_ast_for_instrumentation(
         {:def, meta, [function_head | body]} = ast_node,
         context,
         opts,
         acc
       ) do
  ...
  defp traverse_ast_for_instrumentation(
         {:defp, meta, [function_head | body]} = ast_node,
         context,
         opts,
         acc
       ) do
    ...
    instrumentation_point = %{
      ast_node_id: ast_node_id,
      strategy: :function_boundary,
      # Slightly lower priority for private functions
      priority: 90,
      metadata: %{
    ...
    new_acc =
      if strategy != :none do
        ...
        instrumentation_point = %{
          ast_node_id: ast_node_id,
          strategy: strategy,
          # Medium-low priority for assignments
          priority: 30,
          metadata: %{
    ...
    new_acc =
      if strategy != :none do
        ...
        instrumentation_point = %{
          ...
          metadata: %{
            node_type: node_type,
            # Meta might not have line info for all nodes
            line: 0
          },
          configuration: %{}
        }
  ...
  defp traverse_ast_for_instrumentation(ast_node, context, opts, acc) when is_tuple(ast_node) do
    ...
    # Skip node type and meta
    children = Tuple.to_list(ast_node) |> Enum.drop(2)
